...
┌─────────────────────────────────┐
│        GitHub Actions CI        │
│  1. pytest suite (test job)     │
│  2. docker build (gated on 1)   │
└────────────────┬────────────────┘
                 │  on success, manually push image
                 ▼
        ┌────────────────┐
        │  Amazon ECR    │  (image registry)
        └───────┬────────┘
                │  image pulled by cluster on deploy
                ▼
  ┌─────────────────────────────────────┐
  │           Kubernetes                │
  │                                     │
...
3. **Publish to ECR** — After CI passes, the image is tagged and pushed to an Amazon ECR private repository (`562887204706.dkr.ecr.us-east-1.amazonaws.com/financecs330:latest`).
...
Live app served by AWS EKS at the public LoadBalancer address:
